...
void OFC_Ui::draw_add_time_values() {
    ...
    char buf[32];
    snprintf(buf, sizeof(buf), "+%d min", _add_time_selected_minutes);
    printTFTcentered(buf, _tft->color565(255, 255, 255), 3, 0, 80, 320, 40);

    _tft->fillRect(0, 130, 320, 25, bg);
    if (_add_time_unlimited) {
        printTFTcentered("Max: unlimited", _tft->color565(230, 230, 230), 2, 0, 130, 320, 25);
    } else if (_add_time_max_minutes == 0) {
        printTFTcentered("No extra time available", _tft->color565(255, 200, 200), 2, 0, 130, 320, 25);
    } else if (_add_time_max_minutes > 0) {
        char maxbuf[32];
        snprintf(maxbuf, sizeof(maxbuf), "Max: %d min", _add_time_max_minutes);
        printTFTcentered(maxbuf, _tft->color565(230, 230, 230), 2, 0, 130, 320, 25);
    }
}
